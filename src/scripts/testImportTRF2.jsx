...
import { fetchTRF2Holidays } from '@/utils/trf2HolidayScraper';
import { getMunicipalHolidays } from '@/utils/municipalHolidaysData';
const TestTRF2Import = () => {
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const [nationalHolidays, setNationalHolidays] = useState([]);
    const [stateHolidays, setStateHolidays] = useState([]);
    const [municipalHolidaysSG, setMunicipalHolidaysSG] = useState([]);
    const [municipalHolidaysRJ, setMunicipalHolidaysRJ] = useState([]);
    ...
    const renderHolidayTable = (holidays, title) => (<div className="mb-5">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          ...
          <tbody>
            {holidays.map((holiday, index) => (<tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
    ...
    return (<div className="container mx-auto p-4">
      ...
      {isLoading && (<div className="p-4 mb-4 bg-blue-100 text-blue-700 rounded">
          Carregando feriados... Por favor aguarde.
        </div>)}
      
      {error && (<div className="p-4 mb-4 bg-red-100 text-red-700 rounded">
          {error}
        </div>)}
