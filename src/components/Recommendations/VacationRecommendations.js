...
const VacationRecommendations = forwardRef(({ vacationPeriod, onRecommendationSelect }, ref) => {
    const [recommendations, setRecommendations] = useState([]);
    const [superOptimizations, setSuperOptimizations] = useState([]);
    const [showSuperOptimizations, setShowSuperOptimizations] = useState(false);
    ...
    const getFilteredRecommendations = () => {
        ...
        console.log("Fonte de recomendações:", showSuperOptimizations ? "Super Otimizações" : "Recomendações normais", "Total:", source.length);
        if (activeTab === "all") {
            return source;
        }
    ...
    const EmptyState = () => (<div className="flex flex-col items-center justify-center min-h-[300px] text-center p-4">
      ...
      <div className="border-t border-gray-100 pt-6 w-full">
        ...
        <Button variant="default" className="w-full py-5" onClick={handleShowSuperOptimizations} data-show-super-optimizations>
    ...
    const NoRecommendationsState = () => (<div className="text-center py-8">
      <p className="text-gray-500">Não foram encontradas recomendações para este período</p>
    </div>);
    // No super optimizations state
    const NoSuperOptimizationsState = () => (<div className="text-center py-8">
      <div className="mb-6">
      ...
      <button onClick={() => handleShowSuperOptimizations()} className="mt-4 px-4 py-2 bg-amber-100 text-amber-800 rounded-md hover:bg-amber-200 transition-colors">
        <Zap className="h-4 w-4 mr-2 inline-flex"/>
        Tentar novamente
      </button>
    </div>);
    // Header component
    const RecommendationsHeader = () => (<div className="flex flex-col mb-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-medium">
          {showSuperOptimizations ? 'Super Otimizações' : 'Recomendações'}
        </h2>
        
        {showSuperOptimizations && vacationPeriod && vacationPeriod.isValid && (<Button variant="ghost" size="sm" className="h-9 px-3" onClick={() => setShowSuperOptimizations(false)}>
            Ver recomendações específicas
          </Button>)}
        
        {!showSuperOptimizations && !vacationPeriod && (<Button variant="outline" size="sm" className="h-9 px-3 whitespace-nowrap" onClick={handleShowSuperOptimizations} data-show-super-optimizations>
            <Zap className="h-4 w-4 mr-2 flex-shrink-0"/>
            <span>Ver super otimizações</span>
          </Button>)}
      </div>
      
      {showSuperOptimizations && (<p className="text-sm text-gray-600 mt-2">
    ...
    const FilterTabs = () => {
        ...
        if (isMobile) {
            return (<div className="mb-4 overflow-auto pb-2 -mx-2 px-2">
          <ToggleGroup type="single" value={activeTab} onValueChange={(value) => value && setActiveTab(value)} className="flex w-max space-x-1">
        ...
        return (<Tabs value={activeTab} onValueChange={setActiveTab} className="mb-4">
    ...
    const RecommendationCard = ({ recommendation }) => {
        ...
        return (<Card className="mb-4 hover:shadow-md transition-shadow duration-200">
          ...
          <CardDescription className="text-xs">
            {recommendation.suggestedDateRange.startDate && recommendation.suggestedDateRange.endDate && (<>
                ...
                <div className="flex flex-col mt-1 gap-0.5">
                  ...
                  {metrics.savedVacationDays > 0 && (<span className="flex items-center text-green-600 text-sm font-medium">
        ...
        <CardFooter>
          <Button size="sm" onClick={() => onRecommendationSelect(recommendation.suggestedDateRange, recommendation.type)} className="w-full">
            Aplicar
          </Button>
        </CardFooter>
      </Card>);
    ...
    return (<div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-scale-in" data-vacation-recommendations>
      <div className="p-6">
        ...
        {(vacationPeriod || showSuperOptimizations) && (<>
            ...
            {getFilteredRecommendations().length === 0 ? (showSuperOptimizations ? <NoSuperOptimizationsState /> : <NoRecommendationsState />) : (<div className="space-y-5">
                {getFilteredRecommendations().map(recommendation => (<RecommendationCard key={recommendation.id} recommendation={recommendation}/>))}
              </div>)}
          </>)}
      </div>
    </div>);
