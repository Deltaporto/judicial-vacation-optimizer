...
import { Calendar, DownloadIcon, Info, Sun, Umbrella, Trophy, Star, AlertCircle } from 'lucide-react';
...
const EfficiencyCalculator = ({ vacationPeriod, fractionedPeriods = [], isFractionated = false }) => {
    // Função para detectar e remover sobreposições entre períodos
    const removePeriodOverlaps = (periods) => {
        ...
        for (const period of sortedPeriods) {
            ...
            if (result.length === 0 ||
                ...
                const isValidFraction = result.every(existingPeriod => {
                    ...
                    const hasPartialOverlap = ((period.startDate <= existingPeriod.endDate && period.startDate >= existingPeriod.startDate) ||
        ...
        for (const period of allPeriods) {
            ...
            const isSubsetOfExisting = cleanedPeriods.some(existingPeriod => {
                ...
                return (period.startDate >= existingPeriod.startDate &&
                    period.endDate <= existingPeriod.endDate);
            });
        ...
        return cleanedPeriods.sort((a, b) => a.startDate.getTime() - b.startDate.getTime());
    };
    // For empty state - no periods selected
    if (!vacationPeriod && fractionedPeriods.length === 0) {
        return (<div className="animate-pulse flex flex-col items-center justify-center h-full bg-white rounded-xl shadow-sm border border-gray-100 p-8">
    ...
    const getEfficiencyRating = (efficiency) => {
        if (efficiency >= 1.4)
            return { label: 'Excelente', color: 'text-green-600', icon: <Trophy className="h-5 w-5"/> };
        if (efficiency >= 1.2)
            return { label: 'Ótima', color: 'text-blue-600', icon: <Star className="h-5 w-5"/> };
        if (efficiency >= 1.1)
            return { label: 'Boa', color: 'text-amber-500', icon: <Sun className="h-5 w-5"/> };
    ...
    const getEfficiencyRecommendations = (period) => {
    ...
    const calculateVacationGain = (period) => {
    ...
    const handleExportCalendar = (period) => {
        const icsContent = generateICSFile(period);
        downloadFile(icsContent, `ferias-${formatDate(period.startDate, 'yyyy-MM-dd')}.ics`, 'text/calendar');
    };
    // Single period analysis component
    const renderPeriodAnalysis = (period, index) => {
        ...
        return (<div className={`${index !== undefined ? 'mb-8 pb-8 border-b border-gray-200' : ''}`}>
        ...
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
            ...
            <Button variant="outline" size="sm" className="mt-2 sm:mt-0" onClick={() => handleExportCalendar(period)}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-2">
              {recommendations.map((rec, idx) => (<div key={idx} className="flex items-center text-sm text-gray-600">
    ...
    const calculateCombinedEfficiency = (periods) => {
    ...
    const calculateCombinedVacationGain = (periods) => {
        const validPeriods = periods.filter(Boolean);
        if (validPeriods.length === 0)
            return '0%';
        let totalWorkDays = 0;
    ...
    const renderFractionedSummary = (periods) => {
        // Filter out any null or undefined periods
        const validPeriods = periods.filter(Boolean);
        ...
        return (<div className="mb-6 p-4 bg-blue-50 rounded-lg">
    ...
    if (isFractionated && fractionedPeriods.length > 0) {
        ...
        <div className="p-6">
          ...
          {cleanedPeriods.map((period, index) => (<React.Fragment key={index}>
    ...
    if (!vacationPeriod || !vacationPeriod.isValid) {
        ...
        <div className="p-6 border-b border-gray-100">
          <h2 className="text-lg font-medium flex items-center">
            Análise de Eficiência
            {vacationPeriod && !vacationPeriod.isValid && (<span className="ml-2 text-sm px-2 py-0.5 rounded-full bg-red-100 text-red-600">
                Período Inválido
              </span>)}
          </h2>
        </div>
        
        {vacationPeriod && !vacationPeriod.isValid ? (<div className="p-6 bg-red-50 text-red-600 text-sm">
            <p>{vacationPeriod.invalidReason}</p>
          </div>) : (<div className="animate-pulse flex flex-col items-center justify-center h-full p-8">
            <div className="text-gray-400">
              <Calendar className="h-12 w-12 mb-4 opacity-20"/>
              <p className="text-sm">Selecione um período no calendário para visualizar a análise de eficiência</p>
            </div>
    ...
    return (<div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-scale-in">
      <div className="p-6 border-b border-gray-100">
        <h2 className="text-lg font-medium flex items-center">
          Análise de Eficiência
          {vacationPeriod && (<span className={`ml-2 text-sm px-2 py-0.5 rounded-full ${getEfficiencyRating(vacationPeriod.efficiency).color} bg-opacity-10`}>
