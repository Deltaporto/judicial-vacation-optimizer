...
import { getAllHolidays, addCustomHoliday, removeCustomHoliday, updateCustomHolidays, getCustomHolidays } from '@/utils/holidayData';
...
const HolidayModal = ({ open, onOpenChange, onHolidaysUpdated }) => {
    ...
    const [newHolidayType, setNewHolidayType] = useState('judicial');
    ...
    const filterHolidays = () => {
        ...
        if (searchText) {
            ...
            filtered = filtered.filter(holiday => holiday.name.toLowerCase().includes(searchLower));
        }
        // Ordenar por data
    ...
    const handleHolidaysImported = (importedHolidays) => {
        ...
        const municipalHolidays = importedHolidays.filter(h => h.abrangencia && h.abrangencia.toLowerCase().includes('municipal'));
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
          ...
          <TabsContent value="view" className="flex-1 flex flex-col overflow-hidden">
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <Select value={yearFilter.toString()} onValueChange={(value) => setYearFilter(Number(value))}>
              ...
              <div>
                ...
                <Input id="searchText" value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder="Nome do feriado"/>
              </div>
            </div>
            
            <div className="overflow-y-auto flex-1 border rounded max-h-[50vh]">
              {filteredHolidays.length > 0 ? (<table className="min-w-full">
                  ...
                  <tbody>
                    {filteredHolidays.map((holiday, index) => (<tr key={`${holiday.date}-${index}`} className="border-b hover:bg-muted/30">
                            ...
                            <PopoverContent className="w-80">
                              <div className="space-y-3">
                                ...
                                <div className="flex justify-end space-x-2">
                                  ...
                                  <Button variant="destructive" size="sm" onClick={() => handleDeleteHoliday(holiday)}>
                                    Excluir
                                  </Button>
                                </div>
                              </div>
                ...
                </table>) : (<div className="p-4 text-center text-muted-foreground">
          ...
          <TabsContent value="add" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="holidayName" value={newHolidayName} onChange={(e) => setNewHolidayName(e.target.value)} placeholder="Ex: Aniversário da Cidade"/>
              </div>
            ...
            <div>
              ...
              <Select value={newHolidayType} onValueChange={(value) => setNewHolidayType(value)}>
            ...
            <Button onClick={handleAddHoliday} className="w-full" disabled={!newHolidayName.trim() || !newHolidayDate}>
          ...
          <TabsContent value="import" className="overflow-y-auto">
            <TRF2HolidayImporter existingHolidays={holidays} onImport={handleHolidaysImported}/>
          </TabsContent>
        </Tabs>
