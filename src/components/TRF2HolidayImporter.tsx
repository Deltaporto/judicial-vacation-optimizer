...
import { 
  fetchTRF2Holidays, 
  mergeHolidays, 
  fetchTRF2Municipalities, 
  filterMunicipalHolidaysForMunicipality 
...
interface TRF2HolidayImporterProps {
  existingHolidays: Holiday[];
  onImport: (holidays: Holiday[]) => void;
}

const TRF2HolidayImporter: React.FC<TRF2HolidayImporterProps> = ({
  existingHolidays,
  onImport
}) => {
  const [year, setYear] = useState<number>(new Date().getFullYear());
  const [importState, setImportState] = useState<boolean>(true);
  const [importMunicipal, setImportMunicipal] = useState<boolean>(false);
  const [selectedMunicipality, setSelectedMunicipality] = useState<string>('');
  const [municipalities, setMunicipalities] = useState<string[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [isFetchingMunicipalities, setIsFetchingMunicipalities] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [importedCount, setImportedCount] = useState<number>(0);
  const [importedHolidays, setImportedHolidays] = useState<Holiday[]>([]);
  ...
  const handleMunicipalHolidayImport = async () => {
    ...
    try {
      ...
      const nonMunicipalHolidays = existingHolidays.filter(h => 
  ...
  const handleImport = async () => {
    ...
    try {
      let allImportedHolidays: Holiday[] = [];
      ...
      if (importMunicipal && selectedMunicipality) {
        const municipalCount = allImportedHolidays.filter(h => 
          h.abrangencia && h.abrangencia.includes(`Municipal (${selectedMunicipality})`)
        ).length;
  ...
  return (
    <div className="p-4 border rounded shadow-sm">
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            <Checkbox 
              id="import-state"
              checked={importState}
              onCheckedChange={() => setImportState(!importState)}
              disabled={isLoading}
            />
          ...
          <div className="flex items-center space-x-2">
            <Checkbox 
              id="import-municipal"
              checked={importMunicipal}
              onCheckedChange={() => setImportMunicipal(!importMunicipal)}
              disabled={isLoading}
            />
      ...
      <Button
        ...
        disabled={isLoading || (!importState && !importMunicipal) || (importMunicipal && !selectedMunicipality)}
        className="w-full"
        variant="default"
      >
        {isLoading ? (
