...
const Index = () => {
    ...
    const [splitVacations, setSplitVacations] = useState(null);
    const [activeVacationTab, setActiveVacationTab] = useState("single");
    ...
    const [vacationRecommendations, setVacationRecommendations] = useState([]);
    ...
    const vacationRecommendationsRef = useRef(null);
    ...
    const handleRecommendationSelect = (dateRange, recommendationType) => {
    ...
    const parseShortDate = (shortDate, year) => {
    ...
    const handleHolidaysUpdated = () => {
        ...
        if (selectedRange && isRangeComplete) {
            ...
            if (periodDetails.isValid) {
                const recommendations = generateRecommendations(selectedRange.startDate, selectedRange.endDate, periodDetails);
                setVacationRecommendations(recommendations);
            }
            // Notificar o usuário
            toast({
      ...
      <main className="flex-grow container mx-auto py-8 px-4 sm:px-6 pt-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-6">
              {isRangeComplete && (<Tabs value={activeVacationTab} onValueChange={setActiveVacationTab} className="w-full bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                  ...
                  <TabsContent value="split" className="p-0">
                    {splitVacations && (<EfficiencyCalculator vacationPeriod={null} fractionedPeriods={[
                ...
                ].filter(Boolean)} isFractionated={true}/>)}
                    
                    {/* Display single split period */}
                    {!splitVacations && splitPeriod && (<>
                        {/* Get vacation period details for the split period */}
                        {vacationPeriod && (<EfficiencyCalculator vacationPeriod={null} fractionedPeriods={[
                        vacationPeriod,
                        getVacationPeriodDetails(splitPeriod.startDate, splitPeriod.endDate)
                    ].filter(period => period && period.isValid)} isFractionated={true}/>)}
                      </>)}
                  </TabsContent>
                  
                  <TabsContent value="optimal" className="p-0">
                    {splitPeriods.length > 0 && (<EfficiencyCalculator vacationPeriod={vacationPeriod} fractionedPeriods={[
                    vacationPeriod,
                    ...splitPeriods.map(period => getVacationPeriodDetails(period.startDate, period.endDate))
                ].filter(period => period && period.isValid)} isFractionated={true}/>)}
                  </TabsContent>
                </Tabs>)}
              {!isRangeComplete && (<EfficiencyCalculator vacationPeriod={null}/>)}
            </div>
          </div>
