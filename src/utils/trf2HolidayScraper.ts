 ...
 */
export async function fetchTRF2Holidays(
  year: number = new Date().getFullYear(),
  abrangencia: 'All' | '0' | '1' | '2' = 'All',
  municipality?: string
): Promise<Holiday[]> {
  ...
  try {
    ...
    rows.forEach(row => {
      ...
      if (columns.length >= 3) {
        ...
        let type: 'national' | 'judicial' | 'recess' = 'judicial'; // Padrão
 ...
 */
function extractDatesFromText(dateText: string, year: number): string[] {
  const dates: string[] = [];
 ...
 */
function parseBrazilianDate(dateText: string, year: number): string | null {
  try {
 ...
 */
export async function fetchTRF2Municipalities(
  year: number = new Date().getFullYear()
): Promise<string[]> {
  // Se estiver em desenvolvimento local, use dados simulados
  if (isLocalDevelopment()) {
 ...
 */
function getFallbackMunicipalities(): string[] {
  return [
    'Rio de Janeiro',
    'Niterói',
...
export function mergeHolidays(
  ...
  const municipalHolidays = trf2Holidays.filter(h => 
 ...
 */
export function filterMunicipalHolidaysForMunicipality(
  holidays: Holiday[],
  municipality: string
): Holiday[] {
  ...
  if (filteredHolidays.length === 0) {
    ...
    if (simplifiedMunicipality.length >= 3) {
      ...
      const simplifiedFiltered = holidays.filter(holiday => {
        ...
        return (
          abrangencia.includes(simplifiedMunicipality) ||
