 ...
 */
const findNearbyOpportunities = (vacationPeriod, daysToCheck = 5) => {
    const before = [];
    const after = [];
...
const findPotentialBridges = (year, maxBridgeSize = 3) => {
    const bridges = [];
    ...
    bridges.forEach(bridge => {
        ...
        const checkImportantHolidays = (date) => {
            let bonus = 0;
...
const findOptimalSplitPoint = (vacationPeriod) => {
    if (vacationPeriod.totalDays < 10)
        return null; // Need at least 10 days to split
    let bestSplit = null;
    ...
    for (let i = minPeriodLength - 1; i <= vacationPeriod.totalDays - minPeriodLength; i++) {
        ...
        if (firstPeriod.isValid && secondPeriod.isValid && firstPeriod.workDays > 0 && secondPeriod.workDays > 0) {
            ...
            const combinedEfficiency = (firstPeriod.efficiency * firstWeight) +
            ...
            if (!firstPeriod.isValid)
                console.log(`  - Primeiro período inválido: ${firstPeriod.invalidReason}`);
            if (!secondPeriod.isValid)
                console.log(`  - Segundo período inválido: ${secondPeriod.invalidReason}`);
            if (firstPeriod.workDays === 0)
                console.log(`  - Primeiro período não tem dias úteis`);
            if (secondPeriod.workDays === 0)
                console.log(`  - Segundo período não tem dias úteis`);
        }
    }
    // Debug
    if (bestSplit) {
...
const findOptimalShift = (vacationPeriod, maxSearchWindow = 10 // Aumentei a janela de busca para considerar mais opções
) => {
    let bestShift = null;
    ...
    for (let shift = -maxSearchWindow; shift <= maxSearchWindow; shift++) {
        ...
        if (hasWorkDays && (efficiencyGain > bestEfficiencyGain ||
 ...
 */
export const calculateAdjustedEfficiency = (period) => {
    ...
    if (period.workDays === 0)
        return 1.0; // Eficiência neutra se não há dias úteis consumidos
...
export const findOptimalFractionedPeriods = (year, fractionCount = 6, daysPerFraction = 5) => {
    ...
    const chronologicalPeriods = [...selectedPeriods].sort((a, b) => a.startDate.getTime() - b.startDate.getTime());
...
const selectNonOverlappingPeriods = (sortedPeriods, count) => {
    const selected = [];
    ...
    const idealPeriods = periodsWithWorkDays.filter(period => {
        return (period.startDate.getDay() === 1 && // Segunda-feira
    ...
    while (selected.length < count && i < periodsToUse.length) {
        ...
        const overlaps = selected.some(p => (current.startDate <= p.endDate && current.endDate >= p.startDate));
        if (!overlaps) {
...
export const calculateCombinedEfficiency = (periods) => {
    let totalWorkDays = 0;
    let totalDays = 0;
    let totalNonWorkDays = 0;
...
const calculateBridgeStrategicScore = (startDate, endDate, workDays, holidays = []) => {
    ...
    const isNearImportantDate = (date) => {
        let importance = 0;
        ...
        const daysToRecent = Math.min(Math.abs(differenceInDays(date, judicialRecessStart)), Math.abs(differenceInDays(date, judicialRecessEnd)), Math.abs(differenceInDays(date, prevYearRecessStart)), Math.abs(differenceInDays(date, prevYearRecessEnd)));
        ...
        const daysToHoliday = Math.min(Math.abs(differenceInDays(date, christmas)), Math.abs(differenceInDays(date, newYear)), Math.abs(differenceInDays(date, prevChristmas)));
        ...
        const isNearProvidedHoliday = holidays.some(h => Math.abs(differenceInDays(date, h)) <= 3);
...
export const generateRecommendations = (vacationPeriod) => {
    if (!vacationPeriod.isValid)
        return [];
    ...
    const overlapsWithJudicialRecess = (startDate, endDate) => {
        return ((startDate <= judicialRecessEnd && endDate >= judicialRecessStart));
    };
    ...
    if (optimalFractions && optimalFractions.efficiencyGain > 0.1) { // Only recommend if at least 10% better
        ...
        if (allPeriodsValid) {
            ...
            let periodsDescription = optimalFractions.periods.map((p, i) => `Período ${i + 1}: ${format(p.startDate, 'dd/MM/yyyy')} (${getDayOfWeekName(p.startDate)}) a ${format(p.endDate, 'dd/MM/yyyy')} (${getDayOfWeekName(p.endDate)}) (${p.workDays} dias úteis, ${p.weekendDays + p.holidayDays} não úteis)`).join('; ');
    ...
    if (vacationPeriod.totalDays <= 7) {
        ...
        const yearHolidays = getHolidaysInRange(new Date(currentYear, 0, 1), new Date(currentYear, 11, 31));
        // Função auxiliar para obter nomes de feriados próximos
        const getNearbyHolidayNames = (startDate, endDate) => {
        ...
        const bridgesWithROI = nonOverlappingBridges
            ...
            const strategicScore = calculateBridgeStrategicScore(bridge.startDate, bridge.endDate, bridge.workDays, holidayDates);
    ...
    if (vacationPeriod.totalDays >= 14) {
        ...
        optimalPeriods.forEach(period => {
            recommendations.push({
                ...
                daysChanged: differenceInDays(period.startDate > vacationPeriod.startDate ? period.startDate : vacationPeriod.startDate, period.startDate > vacationPeriod.startDate ? vacationPeriod.startDate : period.startDate)
            });
        });
    }
    ...
    let filteredRecommendations = recommendations.filter(rec => {
        ...
        if (rec.type === 'optimal_fraction' && rec.fractionedPeriods) {
            return !rec.fractionedPeriods.some(period => overlapsWithJudicialRecess(period.startDate, period.endDate));
        }
    ...
    if (filteredRecommendations.length >= 2) {
        ...
        for (let i = 0; i < Math.min(3, filteredRecommendations.length - 1); i++) {
            ...
            for (let j = i + 1; j < Math.min(4, filteredRecommendations.length); j++) {
                ...
                const isCompatible = (compatibleCombinations[rec1.type] && compatibleCombinations[rec1.type].includes(rec2.type)) ||
                    (compatibleCombinations[rec2.type] && compatibleCombinations[rec2.type].includes(rec1.type));
                if (isCompatible) {
                ...
                Depois complemente com fracionamento para maior flexibilidade.`;
                        ...
                        const hybridRecommendation = {
                            id: uuidv4(),
                            type: hybridType, // Usar o tipo específico determinado
                            title: hybridTitle,
                        ...
                        if (!overlapsWithJudicialRecess(hybridRecommendation.suggestedDateRange.startDate, hybridRecommendation.suggestedDateRange.endDate)) {
    ...
    if (optimalFractions && optimalFractions.periods.length > 0) {
        ...
        if (bridgesIntegrated) {
            ...
            const periodsDescription = enhancedFractions
                ...
                .map((p, i) => `Período ${i + 1}: ${format(p.startDate, 'dd/MM/yyyy')} (${getDayOfWeekName(p.startDate)}) a ${format(p.endDate, 'dd/MM/yyyy')} (${getDayOfWeekName(p.endDate)}) - ${p.workDays} dias úteis`).join('; ');
...
const isValidSplit = (firstStart, firstEnd, secondStart, secondEnd) => {
...
export const findOptimalPeriods = (year, length = 15, count = 5) => {
    // Debug
...
export const generateSuperOptimizations = (currentYear = new Date().getFullYear()) => {
    ...
    const createFallbackRecommendations = (year) => {
    ...
    try {
        ...
        years.forEach(year => {
            ...
            const allBridges = findPotentialBridges(year, 10) // Aumentado para 10 dias para capturar mais oportunidades
                ...
                const holidayDates = getHolidaysInRange(new Date(year, 0, 1), new Date(year, 11, 31)).map(h => new Date(h.date));
                const strategicScore = calculateBridgeStrategicScore(bridge.startDate, bridge.endDate, bridge.workDays, holidayDates);
                return {
                    ...bridge,
                    efficiency,
            ...
            const holidays = getHolidaysInRange(new Date(year, 0, 1), new Date(year, 11, 31));
            ...
            superBridges.forEach(bridge => {
                ...
                const hasOverlap = selectedPeriods.some(period => dateRangesOverlap(bridge.startDate, bridge.period.endDate, period.startDate, period.endDate));
                ...
                if (!hasOverlap && formalPeriodDays >= 5) {
                    ...
                    const nearbyHolidays = getHolidaysInRange(subDays(bridge.startDate, 5), addDays(bridge.period.endDate, 5));
                    // Encontrar os dias não úteis conectados antes e depois da ponte
                    let nonWorkDayBefore = null;
                    let nonWorkDayAfter = null;
            ...
            */
            ...
            highHolidayMonths.forEach(month => {
                ...
                if (monthHolidays.length >= 2) {
                    ...
                    if (daysBetween <= 15 && daysBetween >= 3) {
                        ...
                        // Verificar se não há sobreposição com períodos já selecionados
                        const hasOverlap = selectedPeriods.some(period => dateRangesOverlap(startDate, endDate, period.startDate, period.endDate));
                        if (!hasOverlap) {
                            const details = getVacationPeriodDetails(startDate, endDate);
            ...
            miniPeriods.forEach(period => {
                if (miniPeriodsAdded >= 3)
                    return; // Limitar a 3 mini-períodos por ano
                // Verificar se não há sobreposição com períodos já selecionados
                const hasOverlap = selectedPeriods.some(selectedPeriod => dateRangesOverlap(period.startDate, period.endDate, selectedPeriod.startDate, selectedPeriod.endDate));
                if (!hasOverlap) {
                    // Obter nome dos dias da semana para descrição
                    const getDayOfWeekName = (date) => {
                        const days = ['Domingo', 'Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado'];
            ...
            for (const period of optimalHalfPeriods) {
                if (quinzenasAdded >= 3)
                    break; // Limitar a 3 quinzenas por ano
                ...
                const hasSignificantOverlap = selectedPeriods.some(selectedPeriod => {
                    ...
                    const overlap = getOverlapDays(period.startDate, period.endDate, selectedPeriod.startDate, selectedPeriod.endDate);
            ...
            for (const period of optimalPeriods) {
                if (completosAdded >= 2)
                    break; // Limitar a 2 períodos completos por ano
                // Verificar se não há sobreposição significativa com períodos já selecionados
                const hasSignificantOverlap = selectedPeriods.some(selectedPeriod => {
                    // Calcular dias de sobreposição
                    const overlap = getOverlapDays(period.startDate, period.endDate, selectedPeriod.startDate, selectedPeriod.endDate);
            ...
            recessPeriods.forEach(recess => {
                ...
                if (recess.efficiency > 0) {
                    // Verificar se não há sobreposição com períodos já selecionados
                    const hasOverlap = selectedPeriods.some(period => dateRangesOverlap(recess.startDate, recess.endDate, period.startDate, period.endDate));
                    if (!hasOverlap) {
            ...
            if (fractionedOptions && fractionedOptions.periods.length > 0) {
                ...
                const hasSignificantOverlap = fractionedOptions.periods.some(fraction => {
                    return selectedPeriods.some(selectedPeriod => {
                        const overlap = getOverlapDays(fraction.startDate, fraction.endDate, selectedPeriod.startDate, selectedPeriod.endDate);
...
const ensureMinimumVacationDays = (recommendations) => {
...
export const filterAndSortRecommendations = (recommendations, maxRecommendations) => {
...
const checkForWastedVacationDays = (startDate, endDate) => {
    let currentDate = new Date(startDate);
    while (currentDate <= endDate) {
...
const countHolidaysOnWorkDays = (startDate, endDate) => {
    let count = 0;
    let currentDate = new Date(startDate);
    while (currentDate <= endDate) {
 ...
 */
export const calculateHolidayGain = (period) => {
